...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not has_data:
        ...
        with tab4:
            st.markdown("##### Export satisfaction scores")
            final = sat[[msisdn_col,"Eng_Score","Exp_Score","Satisfaction","Sat_Cluster"]]
            st.dataframe(final.head(20).style.format({
                "Eng_Score":"{:.4f}","Exp_Score":"{:.4f}","Satisfaction":"{:.4f}"}),
                use_container_width=True)
            csv = final.to_csv(index=False).encode("utf-8")
            st.download_button(label="⬇ Download satisfaction_scores.csv",
                data=csv, file_name="user_satisfaction_scores.csv", mime="text/csv")
            st.markdown("""
            <div style='background:#0D1B2E;border-left:3px solid #34D399;border-radius:0 8px 8px 0;
                        padding:14px 18px;margin-top:16px;font-size:13px;line-height:1.9;'>
              <b style='color:#34D399;'>MySQL export note (Task 4.6)</b><br>
              <code style='color:#38BDF8;font-size:12px;'>
              from sqlalchemy import create_engine<br>
              engine = create_engine("mysql+pymysql://user:pass@localhost/tellco")<br>
              final.to_sql("user_satisfaction_scores", engine, if_exists="replace", index=False)
              </code>
            </div>
            """, unsafe_allow_html=True)
